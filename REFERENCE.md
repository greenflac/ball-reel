...
**ЛИЦЕНЗИЯ FaceNet — ОТКРЫТЫЙ ВОПРОС ДЛЯ ЮРИСТА.** Код `facenet-pytorch` под
MIT (проверено в `LICENSE.md` установленного пакета, v2.6.0). Веса `vggface2`
обучены на датасете VGGFace2, условия которого ограничивают использование
исследовательскими целями, а продукт коммерческий. Веса участвуют ТОЛЬКО в
отборе обучающего набора, в продукт и в его выход не входят, и заменяемы
(`bar_from_pairs` откалибрует порог под любой другой распознаватель). До
решения юриста считать вопрос закрытым нельзя.
...
- **Офлайн-тесты**: `python3 -m unittest discover -s ball_reel/tests -p 'test_*.py'` → **`Ran 801 tests`, OK (skipped=1)**, ~88 c (последний прогон 2026-08-14; за ту же смену число выросло с 631). ~~148 тестов~~ — из первой редакции памятки.
- **Мутационный аудит** (`codeaudit`, полный прогон 2026-08-14): **77/77, покрытие 100%, выживших нет, «АУДИТ ПРОЙДЕН»**. Он уже дал результат по существу: было обнаружено, что снятый `MIN_COVERAGE` не ронял ни одного теста, из-за чего вердикт `produce` вынесли в отдельную чистую функцию `verdict()`. Отмечено там же: подмена константы через `setattr` не работает для порогов, использованных как значения по умолчанию у аргументов (проверено на `LIMB_WOBBLE_MAX`), поэтому мутация правит ИСХОДНИК копии пакета.
