...
**Шаг прогонялся живьём 2026-08-14** (`nanobanana`, 20 кадров): взято 14 (70%),
итоговый набор 23 кадра / 27 с повторами, предполёт обучения зелёный, план 270
шагов. Модель шлюза выбрана ЗАМЕРОМ трёх кандидатов на одних запросах, а не
умолчанием клиента: `kontext` возвращал одежду и комнату референса во всех
кадрах, `seedream` игнорировал кадрировку 4 раза из 4. Таблица — в
`LORA_RUNBOOK.md` и в шапке `synth`.
