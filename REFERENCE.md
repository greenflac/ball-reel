...
| `lora.py` **(в работе на момент сверки)** | Обучение LoRA личности: конфиг под 6 ГБ, расчёт памяти, предполёт, ПРИЁМКА. Своего цикла обучения не пишет — порождает конфиг и команду для готового тренера, а имена скриптов и ключей сверены с живым апстримом, а не взяты по памяти. Первой проверкой и предполёта, и приёмки вызывает `dataset.independence_report`: если судья участвует в порождении или отборе, обучать бессмысленно. Публичное: `memory_plan`, `reduction_ladder`, `config`, `schedule`, `preflight`, `layout`, `dataset_toml`, `command`, `probe_prompts`, `accept`, `render_acceptance`, `main`. | `dataset`, `device` | GPU (обучение — снаружи) |

Файл `lora.py` на момент сверки не закоммичен и активно правился другим
автором — строка выше снята с рабочего дерева и может разойтись; читать модуль.
