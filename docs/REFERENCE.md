...
считает покрытие по `CORE_MODULES` с порогом 50%; проверяет две парности — копии для мутаций и клона из индекса; затем по очереди портит **122 порога** в КОПИИ пакета
