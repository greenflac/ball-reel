...
| `tests/` (**49 файлов**, `git ls-files ball_reel/tests \| wc -l`) | Арифметика и вердикты без моделей и сети: синтетические скелеты, эмбеддинги, кадры; модельные швы заглушены. Размер набора и число пропусков — `docs/internal/NUMBERS.md`. Здесь стояло «27 файлов, 801 тест» — устарело по обоим. | unittest | нет |
...
записано в коде (`synth`), а не только в переписке — на этом проекте уже
терялись решения по лицензиям, записанные только устно, и код писался против
отменённого запрета.
...
- **Офлайн-тесты**: `python3 -m unittest discover -s ball_reel/tests -p 'test_*.py'` → зелено, пропусков 7 и все семь объяснены в `docs/internal/NUMBERS.md`, где и живёт текущий размер набора. ~~148 тестов~~, ~~631~~, ~~1283~~ — числа из прежних редакций памятки, каждое было верным меньше суток.
