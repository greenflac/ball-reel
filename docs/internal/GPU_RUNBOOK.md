...
гейт — проверено на живых данных и офлайн-тестами: **119 мутаций**, размер
набора см. `docs/internal/NUMBERS.md` (`python3 -m ball_reel.codeaudit`). Здесь стояло
«152 офлайн-теста», потом «801 тест, 56 мутаций» — оба числа из более ранних
сессий, второе разошлось с действительным вдвое по мутациям. Считать надо
прогоном, а не чтением: набор растёт, документ — нет.
