...
COSTS = {
    "пакеты": "1 мс (find_spec, без единого импорта)",
    "диск": "170 мс, из них ~165 — первый импорт huggingface_hub",
    "ffmpeg": "0 мс (shutil.which)",
    "веса": "1 мс (кэш HF уже опрошен строкой выше)",
    "модель позы": "0 мс (stat одного файла)",
    "веса лица": "0 мс на исправной машине; 260 мс в ветке отказа — там "
                 "читается адрес из insightface, чтобы не писать его по памяти",
    "сегментация": "0 мс",
    "DWPose": "0 мс",
    "драйвер": "0 мс здесь (nvidia-smi нет); НЕПРОВЕРЕНО, на машине с картой "
               "это два вызова подпроцесса, ожидаемо сотни мс",
    "условия": "275 мс на 71 png 512x768",
    "driving-кадры": "2 мс на 71 запись манифеста",
    "torch": "1.2-2.4 с — один импорт, и он дороже всего, что выше, вместе",
...
def check_face_model() -> tuple:
    ...
    # Адрес читается из установленной библиотеки, а не по памяти — на этом
    # проекте уже был случай, когда имя репозитория с весами оказалось
    # выдуманным. Импорт стоит четверть секунды, поэтому он здесь, в ветке
    # отказа, а не в начале функции: на исправной машине он не случается.
    try:
