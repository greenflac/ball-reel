...
ЛИЦЕНЗИЯ: ВОПРОС ЗАКРЫТ ВЛАДЕЛЬЦЕМ ПРОДУКТА 2026-08-14, юрист. Записано здесь,
а не только в переписке: на этом проекте уже терялось решение по лицензии,
записанное только устно, и код писался против отменённого запрета.

Факты, на которых решение принималось, остаются как были — их отменить нельзя,
и переспрашивать по ним не нужно:

* код `facenet-pytorch` под MIT (проверено в `LICENSE.md` пакета v2.6.0);
* ВЕСА `vggface2` обучены на датасете VGGFace2, чьи условия ограничивают
  использование исследовательскими целями;
* веса участвуют ТОЛЬКО в отборе обучающего набора и не входят ни в продукт,
  ни в его выход;
* распознаватель заменяем: `bar_from_pairs` откалибрует порог под любой другой,
  и переход стоит одной константы.

Что это НЕ означает: что можно перестать помечать происхождение весов. Смена
распознавателя или вынос его в продукт — новый вопрос, а не продолжение
закрытого.
...
ЖИВОЙ ПРОГОН СОСТОЯЛСЯ (2026-08-14), и он поменял два умолчания. Отбор при
этом не сработал ни разу — все кадры прошли (0.057..0.249 при баре 0.30), то
есть шлюз держит лицо лучше, чем ожидалось. Зато нашлось другое:

1. **Модель по умолчанию была выбрана неверно.** `kontext` вернул платье и
   комнату референса во всех пяти кадрах. Одежда в подпись НЕ ИДЁТ, значит она
   прилипла бы к триггеру, и LoRA выучила бы не человека, а человека в этом
   платье. Заменено на `nanobanana` по замеру — см. константу `MODEL`.
2. **Порядок запросов давал один ракурс.** `variations` перечисляет оси
   подряд, короткий заказ отрезал от списка начало — три кадра различались
   ТОЛЬКО кадрировкой. Починено `_interleaved`.

ЧТО ПО-ПРЕЖНЕМУ НЕ ИЗМЕРЕНО: как ведёт себя отбор на большом заказе (проба
была 3+5+8 кадров) и держится ли доля отсева при 20-30. Доля печатается и
кладётся в отчёт — первый полный набор это и покажет.
...
#: Модель шлюза для image-to-image. ВЫБРАНА ЗАМЕРОМ, а не по умолчанию клиента.
#: Три модели на ОДНИХ И ТЕХ ЖЕ четырёх запросах, 2026-08-14:
#:
#:     модель       лицо p50 / max     кадрировка угадана
#:     kontext        0.175 / 0.249          2/4
#:     nanobanana     0.189 / 0.206          4/4
#:     seedream       0.089 / 0.128          0/4
#:
#: Кадрировка меряется `dataset.measure_frame` (MediaPipe) — тем же прибором,
#: которым потом снимается подпись, а не глазами.
#:
#: `seedream` держит лицо вдвое ближе всех и НЕ ГОДИТСЯ: он игнорирует
#: кадрировку целиком, и набор из него — четыре портрета, то есть ровно тот
#: «один ракурс», против которого весь этот шаг и написан. Число, выигранное по
#: одной оси, здесь ничего не стоит.
#:
#: `kontext` был умолчанием клиента и оказался худшим из трёх: он редактирующий,
#: держится за исходный кадр целиком и вернул платье и комнату РЕФЕРЕНСА во
#: всех пяти кадрах пробы — а одежда в подпись не идёт (`caption_from_measure`
#: меряет кадрировку, поворот и свет), значит прилипла бы к триггеру.
MODEL = "nanobanana"
...
#: Приписка к каждому запросу. ИЗМЕРЕНО, ЧТО ОНА НУЖНА: без неё все три кадра
#: первого живого прогона вышли в платье и комнате РЕФЕРЕНСА — image-to-image
#: держится за исходный кадр целиком, а не только за лицо.
#:
#: Почему это не косметика. Обучающая подпись снимается замером
#: (`dataset.caption_from_frame`) и содержит три оси: кадрировку, поворот и
#: свет. ОДЕЖДЫ среди них нет. Значит одежда, одинаковая во всём наборе, не
#: попадает в подпись и прилипает к триггеру: LoRA выучит не человека, а
#: человека в этом платье и в этой комнате.
#:
#: Личность при этом словами НЕ описывается — её несёт референс. Здесь сказано
#: ровно обратное: что от референса взять (лицо, телосложение), а что менять.
REFERENCE_INSTRUCTION = (
    "keep the same face, the same body build and the same hair; "
    "change the clothing and the surroundings completely")


def _interleaved(rows: list) -> list:
    """Строки `variations` в порядке, где соседние меняют РАЗНЫЕ оси.

    ЗАЧЕМ. `variations` перечисляет оси подряд: сначала все кадрировки, потом
    все повороты, потом свет, потом место. Любой короткий заказ отрезает от
    этого списка начало — то есть ОДНУ ось. Измерено на первом живом прогоне:
    три кадра различались только кадрировкой, а угол, свет и место остались
    базовыми во всех трёх. Набор из такого заказа учит LoRA одному ракурсу
    ровно так же, как девять локальных аугментаций.

    Здесь оси идут по кругу, поэтому разнообразен уже ЛЮБОЙ префикс списка.
    """
    if not rows:
        return []
    base, groups = rows[0], {}
    for r in rows[1:]:
        changed = next((k for k in r if r[k] != base[k]), "")
        groups.setdefault(changed, []).append(r)
    out, keys = [base], list(groups)
...
def prompts_for(count: int, subject: str = "a person") -> list:
    ...
    ОДНОЙ вещью — так на кадр покупается максимум различия. Порядок перебора,
    в отличие от самих осей, наш: см. `_interleaved`.

    Если заказали больше, чем есть комбинаций, список идёт по кругу: повтор оси
    с другим сидом честнее, чем выдуманная девятая ось.
    """
    from .dataset import prompt_for, variations

    rows = _interleaved(variations())
    if not rows:
        return []
    return [f"{prompt_for(rows[i % len(rows)], subject)}. "
            f"{REFERENCE_INSTRUCTION}" for i in range(count)]
