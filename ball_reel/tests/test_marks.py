...
class InkAndSHADINGLiveAtDifferentSCALES(unittest.TestCase):
    ...
    def test_a_SOLID_mark_is_kept_by_the_mass_channel(self):
        ...
        # ВНУТРИ нет — заливка однородна, и частотный канал видит только её
        # край. Окно, целиком лежащее в заливке, для частотного канала пусто,
        # а для массового — максимально контрастно.
        #
        # Первая версия этого теста ставила окно ПО РАЗМЕРУ пятна, и тогда край
        # занимал заметную долю окна: фактура выходила 0.1212, то есть выше
        # порога, и тест «доказывал» обратное тому, что заявлял. Утверждение
        # надо проверять на его собственных условиях, а не на пограничных.
        # Массовый канал здесь намеренно НЕ проверяется, и это честнее, чем
        # подогнать образец: когда заливка шире окна, она накрывает и опорное
        # кольцо, и контраст с окрестностью честно равен нулю — мерить примету
        # против неё же самой нельзя. Это тот самый предел, ради которого в
        # модуле записано «окно задаётся ПОД примету». Что сплошное пятно
        # ловится массой, проверяет `test_a_dark_patch_on_skin_scores_high`.
        import numpy as np

        a = _skin()
        a[50:150, 50:150] = 0.25                  # заливка ШИРЕ окна
        solid = self.m.distinctiveness(a, (70, 70, 130, 130))
        self.assertLess(solid["texture"], self.m.MIN_TEXTURE_CONTRAST)

    def test_the_two_channels_answer_DIFFERENT_questions(self):
        # Прямое сравнение: одинаковая масса чернил, разное распределение.
        # Сплошное пятно — это масса без фактуры; те же чернила, разложенные
        # линиями, — фактура при той же массе. Метрика, у которой один канал,
        # различить их не может, а глаз различает мгновенно.
        import numpy as np

        solid, lined = _skin(), _skin()
        solid[80:120, 80:100] = 0.05              # 800 пикселей одним куском
        for x in range(80, 120, 4):               # столько же, но полосами
            lined[80:120, x:x + 2] = 0.05
        s = self.m.distinctiveness(solid, (70, 70, 130, 130))
        ln = self.m.distinctiveness(lined, (70, 70, 130, 130))
        self.assertAlmostEqual(s["score"], ln["score"], delta=0.05)
        self.assertGreater(ln["texture"], s["texture"] * 1.5)
...
class TheTransferCarriesINKNotTAN(unittest.TestCase):
    ...
    def test_a_COLOURED_mark_arrives_coloured(self):
        # Вторая сторона гашения цветности, и её отсутствие вскрыл мутационный
        # аудит: порог можно было поднять до 99.0 без единого красного теста.
        # Мутация была выбрана мной в безобидную сторону — при 99.0 цветность
        # гасится ПОЛНОСТЬЮ, налёта нет и подавно, а тест на налёт проходит.
        # Настоящий вред другой: цветная татуировка приезжает серой.
        #
        # Отсюда правило, которое дороже самой правки: мутация, выбранная в
        # сторону «стало строже», ничего не доказывает. У порога две стороны, и
        # сторожить надо обе.
        import numpy as np

        source = _skin()
        source[85:115, 85:115] = [0.62, 0.10, 0.12]   # красная тушь
        target = _skin()
        out, rep = self.m.transfer(source, _points(), target, _points(),
                                   self.mark)
        self.assertTrue(rep["applied"], rep["note"])
        moved = np.abs(out - target).sum(axis=-1) > 0.05
        if not moved.any():
            self.skipTest("вклейка ничего не изменила")
        shift = (out - target)[moved].mean(axis=0)
        # Красное отличается от серого тем, что зелёный и синий проседают
        # заметно сильнее красного. Если цветность съедена, все три равны.
        self.assertGreater(float(shift[0] - shift[1]), 0.05,
                           f"цветность съедена: сдвиг по каналам {shift.round(3)}")
