...
class ShippedDataset(unittest.TestCase):
    ...
    ЧЕТВЁРТАЯ редакция, и первая, где есть И личность, И разные позы.
    1 якорь + 16 аугментаций + 21 порождённый = 38 кадров, 42 с повторами.
    Судья принимает 36 из 38 при баре 0.35, медиана 0.208.

    РЕФЕРЕНС ЛИЧНОСТИ — НЕ ЗАГРУЖЕННАЯ ФОТОГРАФИЯ, и это главное решение этого
    набора. Загруженная снята со спины: поворот 0.152 против 0.989 у драйвинга,
    то есть противоположный конец диапазона. Все порождённые кадры стояли от
    неё в 0.418..0.544 — и это читалось как «генератор не держит лицо», пока не
    измерили их ДРУГ ПРОТИВ ДРУГА: 0.208..0.374, 61 пара из 66 в баре.
    Выпадала фотография, а не кадры.

    Референсом взят медоид — самый центральный кадр (медианное расстояние до
    остальных 0.259). Он фронтальный, то есть совпадает по ракурсу с
    драйвингом, и не ретуширован.

    Три предыдущие редакции, для сравнения:
        1) nanobanana,   20 -> 14 (70%), 23 кадра
        2) nanobanana-2, 24 -> 22 (92%), 31 кадр — 9 из 31 у судьи, медиана
           0.459; обученная на ней LoRA дала 0.547, попав в облако набора
        3) только аугментации, 17 кадров — 17 из 17 у судьи, но ОДИН ракурс

    ЦЕНА, названная вслух: верифицируется человек, каким его рисует
    nanobanana-2, а не буквально человек с загруженной фотографии. Решение
    владельца продукта — фотография была намеренно неудачной, и сходство
    ...
    def test_the_step_count_matches_the_number_in_the_runbook(self):
        ...
        pairs = train.load_pairs(self.ROOT)
        self.assertEqual(train.steps_for(pairs, epochs=10), 420)
    ...
    def test_it_is_in_the_git_index_and_not_only_on_the_authors_disk(self):
        ...
        self.assertEqual(
            sum(1 for f in files if f.endswith(".png")), 38,
            "в индексе не все 38 кадров набора")
        self.assertEqual(
            sum(1 for f in files if f.endswith(".txt")), 38,
            "в индексе не все 38 подписей — кадр без подписи тянет на триггер "
            "фон, свет и одежду")
