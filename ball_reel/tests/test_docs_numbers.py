...
в одном файле, `docs/internal/NUMBERS.md`, рядом с командой, которой получено.
...
def _docs() -> list:
    #: HANDOFF_*.md — append-only рабочий журнал (числа в нём исторические
    #: ПО УСТРОЙСТВУ: они фиксируют состояние конкретного прошлого прогона и
    #: не обязаны совпадать с текущим кодом). Живая документация с ним не
    #: смешивается: описание кода мутирует вместе с кодом, журнал — нет.
    out = [ROOT / "README.md"]
    out += sorted(p for p in (ROOT / "docs").glob("**/*.md")
                  if not p.name.startswith("HANDOFF"))
