...
class TheDatasetCanActuallyBeBuilt(unittest.TestCase):
    ...
    def test_a_local_only_set_now_reaches_the_floor(self):
        """ПЕРЕЧЁРКНУТЫЙ ФАКТ: раньше локального набора НЕ ХВАТАЛО.

        ~~реальный кадр плюс восемь преобразований дают 9 при пороге 12~~ —
        это было верно и было главным основанием шага порождения: девять
        кадров при пороге двенадцать, «досыпать порождёнными» стояло советом.

        Перестало быть верным 2026-08-14, и не ради числа. Вскрытие судьёй
        показало, что порождённые кадры для ArcFace ДРУГОЙ ЧЕЛОВЕК
        (0.418..0.544 при баре 0.35), а локальные — тот же с огромным запасом
        (0.000..0.043, проверено на всех шестнадцати). Добирать пришлось тем,
        что судья признаёт, и преобразований стало 16, а набор — 17.

        ЧТО ИЗ ЭТОГО НЕ СЛЕДУЕТ. Что шаг порождения не нужен. Аугментации дают
        РАЗМЕР, но не дают того, ради чего порождение писалось: другого
        ракурса, другого света, другой сцены. Семнадцать вариантов одной
        фотографии — это по-прежнему один ракурс, и LoRA на них переобучится
        на него. Обоснование шага сменилось с «иначе набор мал» на «иначе
        набор однообразен», и это разные утверждения с разной ценой.
        """
        real = self.d.caption_from_frame
        self.d.caption_from_frame = lambda p, **kw: {"caption": "ohwx, x"}
        self.addCleanup(setattr, self.d, "caption_from_frame", real)
        man = self.d.build_dataset(self._photo(), self.dir / "out4")
        self.assertGreaterEqual(man["size"], self.d.MIN_DATASET)
        self.assertEqual(self.d.main(["--face", self._photo(),
                                      "--out", str(self.dir / "out5")]), 0)

    def test_the_local_set_is_still_ONE_ANGLE_and_that_is_its_cost(self):
        """Размер закрыт, разнообразие нет — и путать их нельзя.

        Все кадры локального набора порождены из ОДНОЙ фотографии, то есть
        отличаются масштабом, наклоном, светом и фактурой, но не ракурсом и не
        сценой. Число `MIN_DATASET` про размер и молчит об этом; тест говорит.
        """
        rows = self.d.augmentations()
        self.assertGreaterEqual(len(rows), self.d.MIN_DATASET - 1,
                                "локального набора снова не хватает на порог")
        # Ни одно преобразование не заявляет, что меняет РАКУРС или СЦЕНУ:
        # такое было бы обманом — из одной фотографии их не получить.
        #
        # ПО СЛОВАМ, А НЕ ПО ПОДСТРОКЕ. Первая редакция искала «поз» и нашла
        # его в «экспозиция», забраковав честное описание. Это тот же класс
        # ошибки, из-за которого детектор мёртвых модулей переписан на разбор:
        # подстрока не знает границ слова.
        import re

        for name, why in rows:
            with self.subTest(aug=name):
                words = set(re.findall(r"[а-яё]+", why.lower()))
                for lie in ("ракурс", "ракурса", "сцена", "сцену", "фон",
                            "фона", "поза", "позу", "позы"):
                    self.assertNotIn(lie, words,
                                     f"{name}: аугментация не может дать "
                                     f"{lie!r} — это есть только у съёмки")
