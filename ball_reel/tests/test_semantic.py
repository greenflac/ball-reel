...
class TheArithmeticOfTheAxis(unittest.TestCase):
    ...
    def test_a_claim_without_alternatives_has_no_scale(self):
        """Скор CLIP сам по себе шкалы не имеет — это измерено.

        Текст «человек в спортивной одежде» даёт 0.303 на съёмке, 0.172 на
        верной генерации и 0.199 на кадре в ПЛАТЬЕ: неверный кадр набирает
        больше верного. Утверждение без альтернатив обязано закрываться
        исходом «не смогли», а не превращаться в порог на абсолютном числе.
        """
