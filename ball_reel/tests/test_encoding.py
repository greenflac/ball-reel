...
def _sources() -> list:
    """ВСЕ файлы пакета, ВКЛЮЧАЯ тесты.

    Исключение тестов стояло здесь с обоснованием «тесты работают на временных
    файлах», и обоснование оказалось ложным: временный файл на Windows читается
    той же локалью, что и любой другой. Цена измерена — прогон на Windows дал
    ЧЕТЫРЕ падения `UnicodeDecodeError: 'charmap' codec can't decode byte 0x98`
    ровно в тестах (`test_plumbing` дважды, `test_condition_render`,
    `test_demo_kit`), и все на манифестах, которые сами же и записали в UTF-8.

    То есть сторож был написан против этого дефекта и не смотрел туда, где тот
    и жил. Форма частая: исключение из проверки, оправданное рассуждением, а не
    замером.
    """
