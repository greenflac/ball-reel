...
def write_sequence(paths, indices, out_dir, *, prefer=None,
    ...
    ШИРИНА ПОЛЯ берётся у `fork_video.frame_name` (Е1): дальше по пути кадры
    собираются `sorted(glob('*.png'))`, и своя схема имён сортировалась бы
    иначе ровно там, где этого никто не проверяет.

    НАЧАЛО НУМЕРАЦИИ ВЫБИРАЕТСЯ ЗДЕСЬ, а не берётся оттуда — прежняя редакция
    этой строки указывала неверный источник истины. У `frame_name` начала нет
    вовсе: она форматирует то число, что ей дали, и вызывающих у неё двое —
    `fork_video.decode_argv` кладёт `-start_number 0`, а мы кладём с единицы.
    ИЗМЕРЕНО сторонней сменой на настоящих файлах: порядок `sorted(glob)` у
    двух раскладок совпадает на 7 объёмах из 7 (9, 10, 99, 100, 362, 999,
    1000 кадров), при негативном контроле «без дополнения нулями» он ломается
    на 5 из 6. Номер из имени не разбирает ни один модуль. То есть расходятся
    только ИМЕНА первого кадра (00000 против 00001), и цена расхождения на
    сегодня — ноль; запись оставлена, чтобы следующая смена не искала снова.
    """
    out = Path(out_dir)
