...
def check(desc: dict, *, root=None, prober=None) -> dict:
    ...
    if fps is None:
        ...
        # ~~«Ровно 30, иначе отказ»~~ снято 20.08.2026 вместе с правилом
        # про драйвинг. Поле `fps` карточки — это ЧАСТОТА ВЫХОДА, а она при
        # источнике ниже 30 наследуется: для ролика 24 к/с честное значение
        # здесь 24, и требовать 30 значило бы требовать невозможного.
        # ПОТОЛОК ОСТАЁТСЯ ЖЁСТКИМ: выше 30 мы не отдаём никогда — лишние
        # кадры это время карты без выигрыша в реализме.
        numeric_fps = (isinstance(fps, (int, float))
                       and not isinstance(fps, bool) and fps > 0)
        add(_finding("частота", PASS if numeric_fps and fps <= FPS_OUT
                     else FAIL,
                     f"{fps} к/с"
                     + ("" if numeric_fps and fps <= FPS_OUT else
                        f" выше потолка {FPS_OUT}: выше него не отдаём "
                        f"никогда, лишние кадры это время карты без выигрыша"
                        if numeric_fps else
                        f": частота не число или не положительна")))
        if not numeric_fps:
            # НЕГОДНАЯ ЧАСТОТА НЕ ДОЛЖНА РОНЯТЬ ПРОВЕРКУ. Ниже она делит и
            # умножает (кадры, длина), и на нуле это была трассировка вместо
            # отчёта — то есть оператор видел падение вместо списка находок,
            # а `check` существует ровно чтобы находки СОБРАТЬ ВСЕ СРАЗУ.
            # Тот же класс дефекта уже чинился сегодня в `from_template`.
            # Дальше считаем по штатной частоте и говорим это вслух: числа,
            # посчитанные по подставленной частоте, доверия не заслуживают.
            add(_finding("кадры", UNMEASURED,
                         f"частота {fps!r} негодна — число кадров считать не "
                         f"по чему. Ниже показаны числа по штатным {FPS_OUT} "
                         f"к/с, и они НЕ ПРО ЭТО ОПИСАНИЕ"))
            fps = FPS_OUT
    ...
    if drv is None:
        ...
        if drv_path.is_dir():
            ...
            # ПРАВИЛО ВЛАДЕЛЬЦА 20.08.2026: ниже 30 — наследуем частоту
            # драйвинга, 30 и выше — фиксируем 30. ~~Прежнее «не ниже 30,
            # иначе отказ»~~ снято: оно выросло из посылки «все сурсы будут
            # выше 24», а она не подтвердилась на первом же боевом ролике
            # (24 к/с) — то есть карточка отвергала собственный материал
            # проекта. Правило живёт в `fork_comfy.output_fps` и ТОЛЬКО там
            # (Е1); здесь оно вызывается, а не повторяется.
            out = fork_comfy.output_fps(measured)
            add(_finding("частота драйвинга", out["outcome"],
                         f"{out['note']}. Заявленная в описании частота "
                         f"{fps} к/с"
                         + ("" if fps == out["fps"] else
                            f" РАСХОДИТСЯ с выходной {out['fps']}: длина "
                            f"ролика поедет, поправьте поле `fps`")))
