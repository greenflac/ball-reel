...
def repeat_plan(length, *, fps=None) -> list:
    ...
    ceiling = int(fork_comfy.SECONDS_MAX * fps)
    out = []
...
def pick_finalists(worthy, paths, *, median_score, advantage_min, top=None,
    ...
    dropped_duplicate, dup_unmeasured, budget_hit = 0, 0, False
    for cand in worthy:
        if len(kept) >= top:
            break
        ...
        if tried >= tries:
            ...
            # Иначе — когда оси нет вовсе (ветка выше) — пометить и выпустить
            # правильно: там мы не умеем смотреть, а здесь посмотрели.
            budget_hit = True
            break
        tried += 1
