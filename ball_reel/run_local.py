...
def build_parser():
    ...
    # УМОЛЧАНИЕ 4.0 СТОИЛО ЖИВОГО ПРОГОНА, и вот как. Ветка плана «меньше 5 ГБ»
    # понижает кадр до 384x576 — НИЖЕ РОДНОГО РАЗРЕШЕНИЯ SD1.5, на котором она
    # разваливается: артефакты по всему кадру, вялая амплитуда движения (0.27
    # при баре 0.35) и лицо, которое детектор не находит ни на одном из
    # шестнадцати кадров. Отчёт при этом честно краснел по четырём осям, но
    # НИ ОДНА строка не сказала «дело в разрешении» — план печатался, а его
    # число никто не сверял с тем, что умеет базовая модель.
    #
    # 6.0 — не «побольше на всякий случай», а нижняя граница ветки, дающей
    # 512x768. Карта меньше 5 ГБ существует, и для неё ветка остаётся, но
    # УМОЛЧАНИЕМ она быть не должна: чаще ошибаются в сторону «забыл указать»,
    # чем в сторону «указал лишнее».
    ap.add_argument("--vram", type=float, default=6.0,
                    help="сколько ГБ считать доступными. Ниже 5 план уходит на "
                         "384x576 — SD1.5 там разваливается")
