...
#: она есть в Sapiens2. Раньше здесь стояло «мы её не берём: лицензия
#: запрещает наш сценарий» — УСТАРЕЛО: решение пересмотрено после
#: юридической проверки, модель исполнена на наших кадрах (PRIOR_ART.md,
#: раздел «ПРОВЕРЕНО ЖИВЬЁМ»). Не берём её здесь по ЦЕНЕ, а не по праву:
#: 1.63 ГБ и 12-17 с на кадр против миллисекунд у приближения.
