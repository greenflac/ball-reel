...
отказ, и у него ровно пять причин, каждая из которых встречалась живьём:
нет весов CLIP (~600 МБ, на машине демо их может не быть); в заказе нет
описания слота; ни один кадр не открылся; у утверждения нет ни одной
альтернативы (сравнивать не с чем — шкалы нет); и маржа лежит ВНУТРИ шума
прибора. Последнее — не увёртка: 0.012 это измеренный покадровый sd, и
называть решением разницу, которую прибор не отличает от собственного дрожания,
значит подделывать измерение.
...
#: Все имеющиеся наборы, включая портрет и рисунок скелета, дают ПЛЮС
#: (+0.0202..+0.1049). В репозитории нет ни одного кадра на улице или на пляже,
...
def semantic_match(frames, claims=DEFAULT_CLAIMS, *, model_id: str = MODEL_ID,
    ...
    if not usable:
        return {**empty, "note": (
            ...
            f"(нужно хотя бы {MIN_DECOYS}). Скор CLIP сам по себе шкалы не "
            "имеет: измерено, что неверный кадр набирает на верном описании "
            "больше (0.199), чем верный (0.172). Сравнивать не с чем.")}
    ...
    if args.controls:
        ...
        for c in claims:
            ...
            for name, want, r in controls(args.root, claim=c):
                ...
                mark = "" if r["verdict"] == want else "   <-- РАСХОЖДЕНИЕ"
                print(f"{name:<42}{m:>9}{r.get('frames', 0):>8}  "
                      f"{want} / {r['verdict']}{mark}")
        return 0
