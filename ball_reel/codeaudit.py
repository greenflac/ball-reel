...
MUTATIONS = [
    ...
    ("ball_reel.skeleton", "LIMB_CHAINS", (),
     "частичный ретаргет снова разрешён: бедро сжимается на треть, голень "
     "остаётся донорской, и ControlNet рисует ногу, которой не бывает"),
...
DATA_LINKS = ("kit", "demo", "evidence", "experiments")
...
TREE_ONLY_SKIPS = {
    ...
    "the_staged_tree_holds_what_the_index_holds":
        "проверяет `_stage_clone`, а тот читает `git ls-files`: в копии индекса "
        "нет по устройству",
    "the_staged_tree_does_NOT_hold_what_only_the_disk_holds":
        "то же самое с другой стороны — что в клон НЕ попадает "
        "неотслеживаемое; без индекса вопрос не имеет смысла",
    "the_clone_gets_an_index_of_its_own":
        "проверяет `git init` внутри собранного клона; в копии собирать нечего",
}
...
def _stage_clone(tmp: Path) -> None:
    """Дерево ровно из индекса git — то, что получает склонировавший.

    Никаких ссылок на данные, лежащие рядом: в этом весь смысл. Всё, чего нет в
    индексе, здесь отсутствует, и тесты, опиравшиеся на такие данные, честно
    превращаются в пропуски.
    """
    import shutil
    ...
    for rel in r.stdout.split("\0"):
        if not rel:
            continue
        src, dst = Path(rel), tmp / rel
        if not src.exists():                      # удалён, но ещё в индексе
            continue
        dst.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(src, dst)
    # У КЛОНА ЕСТЬ ИНДЕКС, и без него проверка врёт в свою пользу. Тесты,
    # читающие состав репозитория (`both_kits_are_whole_in_the_git_index` и
    # соседи), вне рабочего дерева git пропускаются — измерено, ровно два
    # лишних пропуска, — и голый каталог с файлами показал бы 9 против 7 там,
    # где настоящий клон даёт 7. Дешевле завести индекс, чем вычитать поправку:
    # поправка устареет молча, а `git add` сам себя пересчитывает.
    for cmd in (["git", "init", "-q"], ["git", "add", "-A"]):
        subprocess.run(cmd, cwd=tmp, capture_output=True, text=True)


def clone_is_as_strong_as_the_tree(here: int | None = None) -> tuple:
    """Молчит ли у склонировавшего то, что говорит у автора.

    ВТОРАЯ САМОПРОВЕРКА, и заведена она по случившемуся дефекту, а не из
    осторожности. Три живых теста ходили в каталог `ref_frames`, которого в
    индексе git нет ни одним файлом: у автора он лежал на диске, тесты были
    зелёными, а в клоне пропускались. Померили — 22 пропуска против 7. Пятнадцать
    сторожей спали у всех, кроме одного человека, и заметить это можно было
    только по числу пропусков, потому что прогон при этом печатает `OK`.

    Проверка копии для мутаций этого НЕ ЛОВИТ по устройству: копия линкует
    данные из рабочего дерева, то есть воспроизводит диск автора, а не индекс.
    Два разных вопроса, и отвечать на них надо порознь.

    Направление ошибки здесь опаснее, чем у мутационной копии: там лишний
    пропуск завышал покрытие, а тут он означает, что проверяющий, ради которого
    репозиторий и открывают, прогоняет набор слабее заявленного.
    """
    import tempfile
    ...
    here = count(None) if here is None else here
    with tempfile.TemporaryDirectory() as tmp:
        _stage_clone(Path(tmp))
        there = count(tmp)
    if there <= here:
        return True, f"в клоне пропущено {there}, в дереве {here} — не слабее"
    return False, (
        f"в клоне пропущено {there} тестов против {here} в дереве: "
        f"{there - here} сторож(а) молчат у всех, кроме автора. Причина одна — "
        f"тест опирается на данные, которых нет в индексе git. Лечится либо "
        f"добавлением данных (`git add -f`, если каталог под .gitignore), либо "
        f"переводом теста на данные, которые в индексе есть. Смягчать сам тест "
        f"нельзя: пропуск не сторожит ничего, а зелёный слабый тест — сторожит "
        f"меньше, чем кажется, и об этом никто не узнает")
...
def main(argv: list) -> int:
    ...
    # И ОТДЕЛЬНО — клон. Копия выше линкует данные с диска автора, поэтому
    # тест, опирающийся на неотслеживаемый каталог, в ней зелёный, а у
    # склонировавшего пропускается. Измерено: 22 пропуска против 7.
    clone_ok, clone_why = clone_is_as_strong_as_the_tree(skipped_here)
    print(f"{'PASS' if clone_ok else 'FAIL'}  клон из индекса    {clone_why}")
    if not clone_ok:
        print("\nОСТАНОВЛЕНО: у склонировавшего набор слабее, чем у автора, — "
              "то есть слабее, чем заявлено.")
        return 1
