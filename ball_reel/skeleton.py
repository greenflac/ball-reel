...
#: Кости, образующие ОДНУ конечность. Ретаргет применяется к цепочке ЦЕЛИКОМ
#: или не применяется вовсе — см. `retarget_plan`.
LIMB_CHAINS = (
...
def retarget_plan(target: dict | None, driving: dict | None) -> tuple:
    ...
    # КОНЕЧНОСТЬ ЦЕЛИКОМ ИЛИ НИКАК. Частичный ретаргет ХУЖЕ отсутствующего:
    # он ломает пропорцию ВНУТРИ конечности, а такого тела не бывает ни у кого.
    #
    # ИЗМЕРЕНО на живом ките, где у цели измерены бёдра и не измерены голени:
    #
    #     нога слева : бедро/голень 0.899 -> 0.630 (множители 0.701 и 1.000)
    #     нога справа: бедро/голень 0.960 -> 0.617 (множители 0.643 и 1.000)
    #
    # То есть бедро сжималось на треть, голень оставалась донорской, и
    # получалась нога с коротким бедром и огромной голенью. ControlNet честно
    # рисовал эту деформацию, а поза потом судилась против НАСТОЯЩЕГО
    # driving-кадра — и мазала системно: 0.26 при баре 0.15, ноль кадров из
    # шестнадцати в баре. Дефект выглядел как «генератор не слушается позы».
    #
    # Руки при этом были целы: там измерены обе кости, и пропорция сдвинулась
    # мягко (1.089 -> 1.217). Разница между двумя случаями и есть правило.
    for chain in LIMB_CHAINS:
        got = [k for k in chain if k in factors]
        if got and len(got) < len(chain):
            missing = [k for k in chain if k not in factors]
            for k in got:
                factors.pop(k)
                origin[k] = (f"снят: в конечности не измерена {missing[0]}, "
                             f"а частичный ретаргет ломает пропорцию внутри "
                             f"конечности")
    return factors, origin
...
def render_sequence(frames: list, out_dir: str | Path, *,
    ...
    для этого мало — на нашей рефке он был правдой ровно на три кости из
    ...
    manifest = {
        ...
        # нашей рефке был правдой ровно на три кости из восьми, и читался
