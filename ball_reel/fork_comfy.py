...
def output_fps(driving_fps: float | None) -> dict:
    """Частота ВЫХОДА по частоте драйвинга. Решение владельца 20.08.2026.

    ПРАВИЛО: меньше 30 — наследуем с драйвинга; 30 и выше — фиксируем 30.

    Откуда оно. ~~«Все сурсы будут выше 24 кадра, вопрос закрыт»~~ — посылка
    НЕ ПОДТВЕРДИЛАСЬ на первом же боевом ролике владельца (720x1278, 24 к/с).
    Выведенное из неё правило «не ниже 30, вверх не приводим» отвергало
    собственный материал проекта. Новое правило не отвергает ничего и при этом
    НИЧЕГО НЕ ВЫДУМЫВАЕТ: вверх по-прежнему не идём, кадров не сочиняем.

    ПОЧЕМУ НАСЛЕДОВАНИЕ, А НЕ ПРИВЕДЕНИЕ. Модель берёт кадры позы один к
    одному: сколько подали, столько родится. Объявить выход тридцатью при
    двадцати четырёх на входе значит ускорить движение на четверть — и в
    отчёте это сойдётся, а заметит клиент. Наследование даёт ровно ту
    скорость, что снята.

    ПОЧЕМУ ПОТОЛОК. Выше тридцати прореживаем: 60 к/с дали бы вдвое больше
    кадров, то есть вдвое больше окон сэмплера и вдвое больше времени карты
    за ту же секунду ролика. Это плата без выигрыша — 30 к/с решением
    владельца признаны достаточными для реализма.

    ЕДИНСТВЕННОЕ МЕСТО, ГДЕ ЭТО ПРАВИЛО ЖИВЁТ (Е1). Скопированное в
    раскодировщик, в карточку и в сводящий проход, оно разъехалось бы в трёх,
    и разъехалось бы тихо: каждый считал бы свою длину.
    """
    if driving_fps is None:
        return {"outcome": UNMEASURED, "fps": None, "inherited": None,
                "note": ("частота драйвинга неизвестна — выходную назвать "
                         "нечем. Это НЕ «берём 30»: подставленная частота "
                         "молча растянет или ускорит движение")}
    if not isinstance(driving_fps, (int, float)) or isinstance(driving_fps, bool):
        raise TypeError(f"частота {driving_fps!r} не число")
    if driving_fps <= 0:
        raise ValueError(f"частота {driving_fps} — не частота")
    if driving_fps < WRAP_FPS:
        return {"outcome": PASS, "fps": driving_fps, "inherited": True,
                "note": (f"драйвинг {driving_fps} к/с — ниже {WRAP_FPS}, "
                         f"выход НАСЛЕДУЕТ частоту источника. Кадры берутся "
                         f"все, ни один не выдуман, движение идёт с той "
                         f"скоростью, с какой снято")}
    return {"outcome": PASS, "fps": float(WRAP_FPS), "inherited": False,
            "note": (f"драйвинг {driving_fps} к/с — {WRAP_FPS} и выше, выход "
                     f"ФИКСИРОВАН на {WRAP_FPS}"
                     + ("" if driving_fps == WRAP_FPS else
                        f"; лишние кадры прореживаются, иначе за ту же "
                        f"секунду ролика платим "
                        f"{driving_fps / WRAP_FPS:.2f}x времени карты"))}
